...
function CellValue({ value, highlight = false }: { value: string | boolean; highlight?: boolean }) {
  if (typeof value === "boolean") {
    return value ? (
      <Check className={`w-4 h-4 sm:w-5 sm:h-5 ${highlight ? "text-cyan-400" : "text-green-400"}`} />
    ) : (
...
export function Comparison() {
  ...
  return (
    <section id="compare" ref={ref} className="relative py-16 sm:py-24 lg:py-32">
      {/* Background */}
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <motion.div
          ...
          className="rounded-xl sm:rounded-2xl border border-[rgba(148,163,184,0.08)] bg-[#0f1115] overflow-hidden"
        >
          {/* Scrollable wrapper for mobile */}
          <div className="overflow-x-auto scrollbar-thin">
            <div className="min-w-[500px] sm:min-w-0">
              {/* Table Header */}
              <div className="grid grid-cols-4 gap-2 sm:gap-4 p-3 sm:p-4 bg-[#0a0b0d] border-b border-[rgba(148,163,184,0.08)]">
                <div className="text-xs sm:text-sm font-medium text-slate-500">Feature</div>
                <div className="text-center">
                  <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl bg-gradient-to-r from-cyan-500/10 to-violet-500/10 border border-cyan-500/20">
                    <Sparkles className="w-3 h-3 sm:w-4 sm:h-4 text-cyan-400" />
                    <span className="font-display text-xs sm:text-sm font-semibold text-slate-100">Studek</span>
                    <Crown className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400" />
                  </div>
                </div>
                <div className="text-center flex items-center justify-center">
                  <span className="text-slate-400 font-medium text-xs sm:text-sm">Anki</span>
                </div>
                <div className="text-center flex items-center justify-center">
                  <span className="text-slate-400 font-medium text-xs sm:text-sm">Quizlet</span>
                </div>
              </div>
              ...
              <div className="divide-y divide-[rgba(148,163,184,0.05)]">
                {comparisonData.map((row, index) => (
                  <motion.div
                    ...
                    className="grid grid-cols-4 gap-2 sm:gap-4 p-3 sm:p-4 hover:bg-[#161a1f]/50 transition-colors"
                  >
